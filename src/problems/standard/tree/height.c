...
int GetHeight(Node *root, int height)
{
    ...
    int left_height = GetHeight(root->left_node, height);
    int right_height = GetHeight(root->right_node, height);
 ...
 * determine and return the height of the tree.
 *
 * The height of a tree is defined as the number
 * of nodes on the longest path from the root
 * node to a leaf node.
 *
 * Example:
 *
...
int Height(BST *bst)
{
    int height = GetHeight(bst->root, 0);
    return height;
}
